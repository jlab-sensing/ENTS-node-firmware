...
from delta_encoding_adc import read_voltage_current_from_csv, process_signal, calculate_lorawan_airtime, calculate_energy_consumption, convert_to_ones_complement, convert_from_ones_complement, adc_to_voltage, adc_to_current
import numpy as np
import statistics
...
def plot_energy_savings_vs_buffer_size(
    ...
    plt.plot(buffer_sizes, energy_savings_percent, marker='o', linestyle='-', color='green')
...
def plot_raw_vs_delta_energy(buffer_sizes, raw_energies, delta_energies):
    ...
    plt.plot(buffer_sizes, delta_energies, label='Delta Encoded', marker='s', color='green')
...
def plot_raw_vs_delta_airtime(buffer_sizes, raw_airtimes, delta_airtimes, raw_airtimes_min, raw_airtimes_max, delta_airtimes_min, delta_airtimes_max):
    """
    ...
    plt.axhline(y=368.9, color='orange', linestyle='--', label='SF7 Max Airtime (368.9 ms)')

    # Plot raw airtime with error bars (min and max)
    plt.errorbar(buffer_sizes, raw_airtimes, 
                 yerr=[np.array(raw_airtimes) - np.array(raw_airtimes_min), np.array(raw_airtimes_max) - np.array(raw_airtimes)], 
                 label='Raw ADC', fmt='o', color='blue', capsize=5, elinewidth=2)

    # Plot delta-encoded airtime with error bars (min and max)
    plt.errorbar(buffer_sizes, delta_airtimes, 
                 yerr=[np.array(delta_airtimes) - np.array(delta_airtimes_min), np.array(delta_airtimes_max) - np.array(delta_airtimes)], 
                 label='Delta Encoded', fmt='s', color='green', capsize=5, elinewidth=2)
...
def plot_adc_readings_and_savings(
    ...
    time_days = np.array([(ts - timestamps[0]).total_seconds() / 86400 for ts in timestamps])
    ...
    ax1.plot(time_days[:len(adc_v_values)], adc_v_values, color='blue', label='Voltage')
    ax2.plot(time_days[:len(adc_i_values)], adc_i_values, color='orange', linestyle='--', label='Current')
    ...
    for buffer_size, data in buffer_results.items():
        ...
        transmission_indices = np.array([i * buffer_size for i in range(len(data.delta_energys))])
        transmission_days = [time_days[min(idx, len(time_days) - 1)] for idx in transmission_indices]
        axs[1].plot(transmission_days, savings_cumulative, label=f"{buffer_size} Buffer Size")


    axs[1].set_title("Cumulative Energy Savings From Delta Encoding at Different Latency Buffer Sizes")
...
def plot_airtime_for_bytes(bytes):
    ...
    plt.axhline(y=368.9, color='orange', linestyle='--', label='Max SF7 Dwell Time (222 byte payload)')
    plt.axhline(y=390.1, color='red', linestyle='--', label='Max SF9 Dwell Time (53 byte payload)')        
...
def analyze_buffering_effect(csv_file, buffer_sizes):
    ...
    measurements = [
        {"ts": int(timestamp.timestamp()), "voltage_delta": int(voltage), "current_delta": int(current)}
    ...
    for i, (timestamp, voltage, current) in enumerate(zip(timestamps, voltages, currents)):
        if previous_voltage is None or previous_current is None:
            ...
            delta_measurements.append({
                "ts": int(timestamp.timestamp()),
                "voltage_delta": convert_to_ones_complement(int(voltage - previous_voltage)),
                "current_delta": convert_to_ones_complement(int(current - previous_current))
            })
    ...
    for buffer_size in buffer_sizes:
        ...
        for i in range(0, len(measurements), buffer_size):
            ...
            energy = calculate_energy_consumption(airtime / 1000)  # Convert ms to seconds
        ...
        print(f"    Average Energy per Transmission: {total_energy_raw / transmissions_raw:.6f} J")
        ...
        print(f"    Average Payload Size: {statistics.mean(delta_payload_sizes):.2f} bytes")
        print(f"    Average Airtime: {total_airtime_delta / transmissions_delta:.2f} ms")
    ...
    delta_energys = [statistics.mean(res.delta_energys) for res in buffer_results.values()]
    raw_airtimes = [statistics.mean(res.raw_airtimes) for res in buffer_results.values()]
    delta_airtimes = [statistics.mean(res.delta_airtimes) for res in buffer_results.values()]
    ...
    plot_raw_vs_delta_airtime(buffer_sizes, raw_airtimes, delta_airtimes,
                              min_raw_airtimes, max_raw_airtimes,
                              min_delta_airtimes, max_delta_airtimes)
    plot_adc_readings_and_savings(
         voltages, currents, timestamps, buffer_results
    )
