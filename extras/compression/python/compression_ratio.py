...
from Algos.Run_length_encoding.run_length_encoding import (
    RunLengthEncodingCompressor,
)  # <-- Import RLE
from Algos.arith_encoding.arith_encoding import (
    ArithmeticEncodingCompressor,
)  # <-- Import Arithmetic Encoding
...
def append_size_ratio(
    results, filename, algorithm, size_original, size_compressed, computation_time
):
...
Arithmetic_compressor = (
    ArithmeticEncodingCompressor()
)  # <-- Initialize Arithmetic Encoding
...
for filepath in sample_messages.iterdir():
    if filepath.is_file() and filepath.name != "__init__.py":
        ...
        # ------------------- LZMA -------------------
        start = time.perf_counter()
        LZMA_compressor.compress(
            str(filepath), compressed_data
        )  # <-- Use LZMA compressor
        end = time.perf_counter()
        ...
        # ------------------- Arithmetic Encoding -------------------
        start = time.perf_counter()
        Arithmetic_compressor.compress(
            str(filepath), compressed_data
        )  # <-- Use Arithmetic Encoding compressor
        end = time.perf_counter()
