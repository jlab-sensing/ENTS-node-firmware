...
def extract_spike_from_csv(
    ...
    print(
        f"🏔️  Spike detected at index {spike_index}, at time {df[time_col].iloc[spike_index]}s"
    )
...
def analyze_spike(csv_file, current_col="I1", time_col="time"):
    ...
    df = pd.read_csv(csv_file)
    drop_times = detect_steep_changes(
        df, current_col="I1", time_col="time", drop_threshold=-50, rise_threshold=50
    )
