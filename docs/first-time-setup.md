...
# Brief
1. Soldering
2. STM32 bootloader (CubeProg option bytes AA using STM32CubeProg), and set CP2102 settings
...
5. Firmware:
    ...
    - ESP32: bootloader and `pio run -e release -t upload --upload-port ZZZ`
        - Save join MAC in build log. The AP MAC is +1
    - STM32: `pio run -e stm32 -t upload -t monitor --upload-port XXX --monitor-port YYY`
        - Save LoRaWAN keys "######"
6. Register logger
7. Create cell
8. Wifi userConfig: `ents-<DevEUI>` default pw `ilovedirt`, go to http://192.168.4.1, submit and STM32 RESET
...
0. Prerequisites:
    - Software:
        ...
        - Download and install [Simplicity Studio v5](https://www.silabs.com/software-and-tools/simplicity-studio/simplicity-studio-version-5).
            - Alternatively, you may use [the command line tool](https://www.silabs.com/documents/public/example-code/AN721SW.zip).
            - You cannot use Simplicity Studio v6, which does not have the USBXpress Configurator tool.
    - Hardware:
...
2. Clear STM32 bootloader and configure the CP2102N USB to serial converter
    - Prerequistes
    ...
    - Enable USB UART transmit and receive activity LEDs for the CP2102N.
        - Launch Simplicity Studio v5.
        - With the ENTS board plugged into your computer via a USB cable, follow the guide in [SiLab AN721](https://www.silabs.com/documents/public/application-notes/AN721.pdf) to use the tool to configure the CP2102. Take note of Chapter 3 "Device Customization Software" and Figure 3.11 "GPIO Customization".
            - The only changes from the default/reset settings should be:
                - (Optional) Check the box for "Use Internal Serial Number". Alternatively, you can customize the Serial String value as you wish.
                - GPIO0: Alternate Function set to "TX Toggle".
                - GPIO1: Alternate Function set to "RX Toggle".
