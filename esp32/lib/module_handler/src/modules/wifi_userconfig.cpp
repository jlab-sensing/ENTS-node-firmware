...
void ModuleUserConfig::OnReceive(const Esp32Command &cmd) {
    ...
    else if (current_command_.type == UserConfigCommand_RequestType_RESPONSE_CONFIG && 
...
size_t ModuleUserConfig::OnRequest(uint8_t *buffer) {
    if (current_command_.type == UserConfigCommand_RequestType_REQUEST_CONFIG) {
        ...
        if (has_config_) {
            memcpy(&response.command.user_config_command.config_data, &current_config_, sizeof(UserConfiguration));
        }
        ...
        Log.noticeln(" Successfully encoded configuration (%d bytes)", stream.bytes_written);
...
void ModuleUserConfig::printReceivedConfig() {
    ...
    if (current_config_.Upload_method == Uploadmethod_WiFi) {
        ...
        Log.noticeln("   API Endpoint: %s:%u", 
                    current_config_.API_Endpoint_URL, 
...
void ModuleUserConfig::updateWebConfig(const UserConfiguration *pb_config) {
    ...
    config.upload_method = (pb_config->Upload_method == Uploadmethod_LoRa) ? "LoRa" : "WiFi";
    ...
    for (int i = 0; i < pb_config->enabled_sensors_count; i++) {
        switch(pb_config->enabled_sensors[i]) {
            case EnabledSensor_Voltage: config.voltage_enabled = true; break;
            case EnabledSensor_Current: config.current_enabled = true; break;
            case EnabledSensor_Teros12: config.teros12_enabled = true; break;
            case EnabledSensor_Teros21: config.teros21_enabled = true; break;
            case EnabledSensor_BME280: config.bme280_enabled = true; break;
        }
    }
