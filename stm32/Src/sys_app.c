...
uint8_t GetBatteryLevel(void) {
  ...
  /* Convert battery level from mV to linear scale: 1 (very low) to 254 (fully
   * charged) */
  if (batteryLevelmV > VDD_BAT) {
    ...
    batteryLevel = (((uint32_t)(batteryLevelmV - VDD_MIN) * LORAWAN_MAX_BAT) /
                    (VDD_BAT - VDD_MIN));
  }
...
static void TimestampNow(uint8_t *buff, uint16_t *size) {
  ...
  tiny_vsnprintf_like((char *)buff, MAX_TS_SIZE, "%ds%03d:", curtime.Seconds,
                      curtime.SubSeconds);
#else
