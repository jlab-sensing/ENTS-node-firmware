 ...
 * @note Please note that LORAWAN_DEFAULT_DATA_RATE is used only when
 * LORAWAN_ADR_STATE is disabled
 */
#define LORAWAN_DEFAULT_DATA_RATE DR_0

/*!
 * LoRaWAN Default Tx output power
 * @note LORAWAN_DEFAULT_TX_POWER must be defined in the [XXXX_MIN_TX_POWER -
 XXXX_MAX_TX_POWER] range, else the end-device uses the XXXX_DEFAULT_TX_POWER
 value
 */
 ...
 * @note useful only when context management is enabled by
 ...
 *
 * \remark periodicity is equal to 2^LORAWAN_DEFAULT_PING_SLOT_PERIODICITY
 * seconds example: 2^4 = 16 seconds. The end-device will open an Rx slot every
 * 16 seconds.
 */
