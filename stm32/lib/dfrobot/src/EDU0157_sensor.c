...
int EDU0157MeasureAll(EDU0157Data *sensor_data) {
  ...
  int ret = sscanf(value,
                   "WindSpeed:%f m/s, WindDirection:%3[^,], Altitude:%f m, "
                   "Pressure:%f hPa, Temp:%f C, Humi:%f %%RH",
                   &sensor_data->wind_speed, direction, &sensor_data->altitude,
